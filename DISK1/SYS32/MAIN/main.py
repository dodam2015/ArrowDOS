try:
    ...
    def log_error(exc_type, exc_value, exc_tb):
        ...
        filename = f"{date}-{ampm}{time_str}.txt"
    ...
    # target도 timezone-aware로 만들어야 함
    target_naive = datetime.datetime(TARGET_YEAR, TARGET_MONTH, TARGET_DAY,
                                     TARGET_HOUR, TARGET_MINUTE, TARGET_SECOND)
    target = target_naive.replace(tzinfo=datetime.timezone(datetime.timedelta(hours=9)))
