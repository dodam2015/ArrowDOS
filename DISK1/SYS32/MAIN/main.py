    ...
    while True:
        ...
        if cmd=='?':
            ...
            try:
                ...
                print('취소되었습니다.')
        elif cmd.startswith('restore point(') and cmd.endswith(')'):
            if cmd[14:-1]=='new':
                print("시스템 복원 지점 생성 마법사를 불러오는 중 입니다...\n")
                time.sleep(1)
                tmp=os.path.join(dir, 'SYS32', 'SYSdriver', 'Zipper', 'Zipper.py')
                subprocess.run([sys.executable,tmp])
            elif cmd[14:-1]=='load':
                print()
            else:
                print('restore point(new) - 시스템 복원 지점을 생성합니다.')
                print('restore point(load) - 시스템 복원 지점을 불러옵니다.')
